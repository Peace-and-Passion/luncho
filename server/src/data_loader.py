...
def load_data(url: str, filename: str, processor: Callable[[dict[str, Any]|None, str], bool], force_download: bool = False, use_dummy_data: bool = False, ) -> None:
    ''' Loads data from API, filename in GCS or local file system.
       ...
       Args:
          ...
          processor:       Data processor. It will be passed data and a string representing its source,
                           such like processor(data, 'source'). If data is not availabe, None is passed.
                           If it returns False, falls back to the next data source.
    ...
    if data_saved and not force_download:
        ...
        if datetime.datetime.utcnow().date() == timestamp_date:  # today?
            if processor(data_saved, 'backup file'):
                return
